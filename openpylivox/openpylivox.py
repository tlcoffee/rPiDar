...
class _dataCaptureThread(object):
            ...
            if version == 5:
                ...
                if lenData > 0:
                    ...
                    if self.showMessages: print(
                        "   " + self.sensorIP + " --> writing data to file: " + self.filePathAndName)
                    ...
                    if self.showMessages: print(
                        "   " + self.sensorIP + " --> WARNING: no point cloud data was captured")
    ...
    def statusCodes(self):
        ...
        return [self.system_status, self.temp_status, self.volt_status, self.motor_status, self.dirty_status,
                self.firmware_status, self.pps_status, self.device_status]
...
class openpylivox(object):
    ...
    _CMD_WRITE_ZERO_EXTRINSIC = bytes.fromhex(
                    ...
                    if ack == "ACK (response)" and cmd_set == "General" and cmd_id == "1":
                        ...
                        if ret_code == 0:
                            ...
                            self._heartbeat = _heartbeatThread(1, self._cmdSocket, self._sensorIP, 65000,
                                                               self._CMD_HEARTBEAT, self.showMessages)
                            time.sleep(0.15)
                            self._query()
                            if self.showMessages: print(
                                "Connected to Mid-40 sensor at IP: " + self._sensorIP + " (F/W: " + self._firmware + ")")
                        else:
    ...
    def disconnect(self):
        ...
        if self._isConnected:
            ...
            try:
                ...
                if self.showMessages: print(
                    "*** Error trying to disconnect from Mid-40 sensor at IP: " + self._sensorIP)
        else:
            ...
            if select.select([self._cmdSocket], [], [], 0.1)[0]:
                ...
                if ack == "ACK (response)" and cmd_set == "Lidar" and cmd_id == "0":
                    ...
                    if ret_code == 1:
                        ...
                        if self.showMessages: print(
                            "   " + self._sensorIP + " --> lidar is spinning up, please wait...")
                        while True:
                            time.sleep(0.1)
    ...
    def dataStart(self):
        ...
        if self._isConnected:
            if not self._isData:
                self._captureStream = _dataCaptureThread(self._sensorIP, self._dataSocket, "", 0, 0, 0,
                                                         self.showMessages)
                time.sleep(0.12)
                self._waitForIdle()
                ...
                if select.select([self._cmdSocket], [], [], 0.1)[0]:
                    ...
                    if ack == "ACK (response)" and cmd_set == "General" and cmd_id == "4":
                        ...
                        if self.showMessages: print(
                            "   " + self._sensorIP + " --> incorrect start data stream response")
            else:
            ...
            if select.select([self._cmdSocket], [], [], 0.1)[0]:
                ...
                if ack == "ACK (response)" and cmd_set == "General" and cmd_id == "8":
                    ...
                    if ret_code == 0:
                        ...
                        if self.showMessages: print(
                            "   " + self._sensorIP + " --> FAILED to change to dynamic IP (DHCP assigned)")
                else:
                    if self.showMessages: print(
                        "   " + self._sensorIP + " --> FAILED to change to dynamic IP (DHCP assigned)")
                ...
                if select.select([self._cmdSocket], [], [], 0.1)[0]:
                    ...
                    if ack == "ACK (response)" and cmd_set == "General" and cmd_id == "8":
                        ...
                        if ret_code == 0:
                            ...
                            if self.showMessages: print(
                                "   " + self._sensorIP + " --> FAILED to change static IP (must be " + ipRange + ")")
                    else:
                        if self.showMessages: print(
                            "   " + self._sensorIP + " --> FAILED to change static IP (must be " + ipRange + ")")
            else:
        ...
        if self._isConnected:
            ...
            if select.select([self._cmdSocket], [], [], 0.1)[0]:
                ...
                if ack == "ACK (response)" and cmd_set == "General" and cmd_id == "5":
                    ...
                    if ret_code == 1:
                        if self.showMessages: print(
                            "   " + self._sensorIP + " --> FAILED to set Cartesian coordinate output")
                    elif ret_code == 0:
                        self._coordSystem = 0
                else:
                    if self.showMessages: print(
                        "   " + self._sensorIP + " --> incorrect change coordinate system response (Cartesian)")
        else:
            if self.showMessages: print("Not connected to Mid-40 sensor at IP: " + self._sensorIP)
        ...
        if self._isConnected:
            ...
            if select.select([self._cmdSocket], [], [], 0.1)[0]:
                ...
                if ack == "ACK (response)" and cmd_set == "General" and cmd_id == "5":
                    ...
                    if ret_code == 1:
                        if self.showMessages: print(
                            "   " + self._sensorIP + " --> FAILED to set Spherical coordinate output")
                    elif ret_code == 0:
                        self._coordSystem = 1
                else:
                    if self.showMessages: print(
                        "   " + self._sensorIP + " --> incorrect change coordinate system response (Spherical)")
        else:
            if self.showMessages: print("Not connected to Mid-40 sensor at IP: " + self._sensorIP)
        ...
        if self._isConnected:
            ...
            if select.select([self._cmdSocket], [], [], 0.1)[0]:
                ...
                if ack == "ACK (response)" and cmd_set == "Lidar" and cmd_id == "1":
                    ...
                    if ret_code == 1:
                        if self.showMessages: print(
                            "   " + self._sensorIP + " --> FAILED to set extrinsic parameters to zero")
                    elif ret_code == 0:
                        self.readExtrinsic()
                else:
                    if self.showMessages: print(
                        "   " + self._sensorIP + " --> incorrect set extrinsics to zero response")
        else:
            if self.showMessages: print("Not connected to Mid-40 sensor at IP: " + self._sensorIP)
            ...
            if goodValues:
                ...
                if select.select([self._cmdSocket], [], [], 0.1)[0]:
                    ...
                    if ack == "ACK (response)" and cmd_set == "Lidar" and cmd_id == "1":
                        ...
                        if ret_code == 1:
                            if self.showMessages: print(
                                "   " + self._sensorIP + " --> FAILED to set extrinsic parameters")
                        elif ret_code == 0:
                            self.readExtrinsic()
                    else:
                        if self.showMessages: print(
                            "   " + self._sensorIP + " --> incorrect set extrinsic parameters response")
        else:
            if self.showMessages: print("Not connected to Mid-40 sensor at IP: " + self._sensorIP)
        ...
        if self._isConnected:
            ...
            if select.select([self._cmdSocket], [], [], 0.1)[0]:
                ...
                if ack == "ACK (response)" and cmd_set == "Lidar" and cmd_id == "3":
                    ...
                    if ret_code == 1:
                        if self.showMessages: print(
                            "   " + self._sensorIP + " --> FAILED to set rain/fog suppression value")
                else:
                    if self.showMessages: print(
                        "   " + self._sensorIP + " --> incorrect set rain/fog suppression response")
        else:
            if self.showMessages: print("Not connected to Mid-40 sensor at IP: " + self._sensorIP)
        ...
        if self._isConnected:
            if self._isData:
                if self._firmware != "UNKNOWN":
                    ...
                    if duration < 0:
                        if self.showMessages: print(
                            "   " + self._sensorIP + " --> * ISSUE: saving data, negative duration")
                    else:
                        # max duration = 4 years - 1 sec
                        if duration >= 126230400:
                            if self.showMessages: print(
                                "   " + self._sensorIP + " --> * ISSUE: saving data, duration too big")
                        else:

                            if secsToWait < 0:
                                if self.showMessages: print(
                                    "   " + self._sensorIP + " --> * ISSUE: saving data, negative time to wait")
                            else:
                                # max time to wait = 15 mins
                                if secsToWait > 900:
                                    if self.showMessages: print(
                                        "   " + self._sensorIP + " --> * ISSUE: saving data, time to wait too big")
                                else:

                                    if filePathAndName == "":
                                        if self.showMessages: print(
                                            "   " + self._sensorIP + " --> * ISSUE: saving data, file path and name missing")
                                    else:
                ...
                if self.showMessages: print(
                    "   " + self._sensorIP + " --> WARNING: data stream not started, no CSV file created")
